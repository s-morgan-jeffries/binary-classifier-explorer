...
const gaussian = (mean, stdev, x) => {
  const a = 1 / (stdev * Math.sqrt(2 * Math.PI));
  const b = -1 * (Math.pow(x - mean, 2) / (2 * Math.pow(stdev, 2)));
  return a * Math.exp(b);
};

const SliderControl = ({ label, value, onChange, min, max, step = 1 }) => (
...
const MetricDisplay = ({ name, formula, calculation, value }) => (
...
const ClassificationDemo = () => {
  ...
  useEffect(() => {
    const newData = [];
    for (let x = 0; x <= 100; x++) {
      const positiveY = gaussian(positiveMean, positiveSd, x) * prevalence;
      const negativeY = gaussian(negativeMean, negativeSd, x) * (1 - prevalence);
  ...
  const calculateMetrics = () => {
    ...
    // Ensure the sum is exactly TOTAL_POPULATION
    const diff = TOTAL_POPULATION - (tp + fn + fp + tn);
    return { tp: tp + diff, fn, fp, tn };
  };

  const { tp, fn, fp, tn } = calculateMetrics();
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const positiveTotal = Math.round(data.reduce((sum, d) => sum + d.positiveY, 0) * TOTAL_POPULATION);
      const negativeTotal = Math.round(data.reduce((sum, d) => sum + d.negativeY, 0) * TOTAL_POPULATION);
      return (
        <div className="bg-white border border-gray-300 p-2 shadow-md">
          <p className="text-purple-500">Population with Condition: {positiveTotal.toLocaleString()}</p>
          <p className="text-green-500">Population without Condition: {negativeTotal.toLocaleString()}</p>
        </div>
      );
    }
    return null;
